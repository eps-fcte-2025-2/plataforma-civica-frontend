describe('Denúncia de Esquema de Manipulação', () => {
  ...
  it('deve criar uma denúncia de esquema de manipulação com múltiplas partidas', () => {
    ...
    cy.get('input[placeholder="Nome da partida"]').type('Flamengo x Vasco - Campeonato Carioca');
    cy.get('input[type="date"]').type('2024-11-20');
    
    // Adicionar segunda partida
    cy.get('button').contains('Adicionar Partida').click();
    cy.get('input[placeholder="Nome da partida"]').eq(1).type('Botafogo x Fluminense - Copa do Brasil');
    cy.get('input[type="date"]').eq(1).type('2024-11-25');
    
    // Adicionar terceira partida
    cy.get('button').contains('Adicionar Partida').click();
    cy.get('input[placeholder="Nome da partida"]').eq(2).type('Flamengo x Botafogo - Brasileirão');
    cy.get('input[type="date"]').eq(2).type('2024-12-01');
    ...
    cy.get('input[placeholder="Nome do clube"]').eq(0).type('Flamengo');
    
    cy.get('button').contains('Adicionar Clube').click();
    cy.get('input[placeholder="Nome do clube"]').eq(1).type('Botafogo');
    ...
    cy.get('textarea').type(
      'Esquema complexo de manipulação de resultados envolvendo múltiplos jogos do futebol carioca. ' +
      'O organizador Carlos Manipulador coordena uma rede que inclui jogadores, árbitros e apostadores profissionais. ' +
      'As partidas são previamente combinadas com resultados específicos, incluindo número de cartões, ' +
      'escanteios e até mesmo o minuto exato de gols. O esquema opera há pelo menos 6 meses e já ' +
      'manipulou dezenas de partidas. Há evidências de transferências bancárias suspeitas e ' +
      'comunicações via aplicativos de mensagem entre os envolvidos. O prejuízo estimado às ' +
      'casas de apostas ultrapassa R$ 5 milhões.'
    );
    ...
    cy.wait('@criarDenunciaEsquema').then((interception) => {
      ...
      expect(requestBody.pontualOuDisseminado).to.equal('DISSEMINADO');
      ...
      expect(requestBody.pessoasEnvolvidas).to.have.length(4);
  ...
  it('deve remover partidas, clubes e pessoas adicionados', () => {
    ...
    cy.get('input[type="date"]').eq(1).type('2024-12-02');
    
    cy.get('button').contains('Adicionar Partida').click();
    cy.get('input[placeholder="Nome da partida"]').eq(2).type('Partida 3');
    cy.get('input[type="date"]').eq(2).type('2024-12-03');
